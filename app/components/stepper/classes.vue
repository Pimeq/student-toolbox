<script setup lang="ts">
	...
	watch(selectedRows, (rows) => {
		...
		for (const [key, isSelected] of Object.entries(rows || {})) {
			...
			const foundById = (classesList.value || []).find(
			...
			const foundByIndex = (classesList.value || [])[index]
	...
	const { data: classesList } = useAsyncData("classes", async () => {
		const coursesIds = model.value?.courses || []
		if (coursesIds.length === 0) return []

		const courseGroups = await Promise.all(
			coursesIds.map(async (courseId) => {
				const { data, error } = await client.rpc("get_classes", {
