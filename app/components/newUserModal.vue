				...
				<div class="w-full">
					<UStepper
						...
						<template #content="{ item }">
							...
							<StepperFaculties
								v-if="item.title == 'Faculties'"
								v-model:items="enrollItems" />
							<StepperCourses
								v-if="item.title == 'Courses'"
								v-model:items="enrollItems" />
							<StepperClasses
								v-if="item.title == 'Classes'"
								v-model:items="enrollItems" />
							...
							<div class="flex gap-2 justify-between mt-4">
								...
								<UButton
									...
									:disabled="!stepper?.hasNext || !canGoNext(item.title)"
									@click="stepper?.next()">
									Next
								</UButton>
								<UButton
