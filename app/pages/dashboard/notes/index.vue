			...
			<div class="mb-5 rounded-xl border border-gray-200 dark:border-gray-800 bg-white/90 dark:bg-gray-900/90 backdrop-blur p-3">
				<div class="flex flex-wrap items-center justify-between gap-3">
					...
					<div class="flex items-center gap-2">
						...
						<UButton :loading="isCreating" icon="i-lucide-plus" size="sm" color="black" variant="solid" @click="handleAddNote">Nowa Notatka</UButton>
					</div>
				</div>
			</div>

			<!-- Pusty stan (Gdy nie ma notatek) -->
			<div v-if="notes.length === 0" class="flex flex-col items-center justify-center h-full text-center space-y-4">
				...
				<UButton :loading="isCreating" icon="i-lucide-plus" color="black" variant="solid" @click="handleAddNote">Dodaj Notatkę</UButton>
			</div>

			<div v-else class="space-y-8">
				<section v-for="section in noteSections" :key="section.key" class="space-y-3">
					<div class="flex items-center justify-between">
						<h2 class="text-sm font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ section.label }}</h2>
						<span class="text-xs px-2 py-1 rounded-md border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300">
							{{ section.notes.length }}
						</span>
					</div>

					<div v-if="section.notes.length === 0" class="text-sm text-gray-500 dark:text-gray-400 bg-white/70 dark:bg-gray-900/30 border border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-4">
					...
					<div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
						...
						>
							...
							>
								<template #header>
									<div class="flex items-start justify-between gap-3">
										<div v-if="isSelectionMode" class="pt-1" @click.stop>
											<UCheckbox :model-value="isNoteSelected(note.id)" @update:model-value="setNoteSelection(note.id, !!$event)" />
										</div>
										<div class="flex items-center justify-center p-2 rounded-lg border" :class="colorMap[note.color || 'blue']">
										...
										<div class="flex-1 min-w-0">
											<div class="flex items-center justify-between gap-2 mb-1" @click.stop>
												<UButton
													...
													:disabled="isSelectionMode || !note.is_owner"
											...
											<div v-if="isEditingTitle === note.id" class="w-full flex gap-1 -mt-1 -ml-1">
												<UInput
												...
												/>
												<UButton icon="i-lucide-check" color="green" size="xs" variant="ghost" @click.stop="saveTitle(note)" />
											</div>
											<div v-else class="flex items-center justify-between w-full group/title">
												<h3 class="font-semibold text-gray-900 dark:text-white truncate" @click.stop="startEditingTitle(note)">{{ note.title }}</h3>
												<UButton
								...
								<template #footer>
									<div class="relative z-10 flex items-center justify-between w-full text-xs font-medium text-gray-400">
...
<UButton color="black" @click="confirmGroupSelection" :disabled="!selectedGroupId || isFetchingGroups">Zatwierdz</UButton>
</div>
</template>
</UCard>
</template></UModal>
